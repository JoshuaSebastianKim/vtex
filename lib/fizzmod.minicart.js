...
(function($, Fizzmod, checkout, window, undefined) {
	let Minicart = {
		...
		init: function() {
			...
			this.getOrderForm()
				.then(orderForm => this._trigger(this.events.INIT, orderForm))
		...
		getOrderForm: function() {
			...
			// IF ORDERFORM IS STORED IN CACHE RETURN IT
			if(this.orderForm) {
				return $.Deferred().resolve(this.orderForm).promise();
			}

			if(this.getOrderFormActiveRequest) {
				xhr = this.getOrderFormActiveRequest;
			} else {
				xhr = this.getOrderFormActiveRequest = checkout.getOrderForm();
			}
			...
			xhr.done((orderForm) => {
				...
				this.getOrderFormActiveRequest = undefined;
			});
